// src/AssetsEditor/components/LeftPanel.tsx
    ...
    return (
        <div className="h-full flex flex-col w-[260px] ml-4 transition-all duration-300 gap-4">
            ...
            <div className="glass-panel p-4 border border-white/10 bg-black/40 flex flex-col gap-3 shrink-0">
                ...
                <div className="grid grid-cols-3 gap-2 overflow-y-auto max-h-[240px] pr-1 custom-scrollbar">
                    {frames.map((frame, index) => (
                        <div
                            key={frame.id}
                            onClick={() => selectFrame(index)}
                            className={`
                        aspect-square border-2 cursor-pointer relative overflow-hidden group rounded-sm
                        transition-all duration-200
                        ${currentFrameIndex === index ? 'border-blue-500 shadow-[0_0_10px_rgba(59,130,246,0.5)] z-10 scale-105' : 'border-white/10 hover:border-white/30'}
                    `}
                        >
                            ...
                            {/* Hover Actions */}
                            <div className="absolute inset-0 bg-black/80 opacity-0 group-hover:opacity-100 flex flex-col items-center justify-center gap-1 transition-opacity backdrop-blur-[1px]">
                                <div className="text-[10px] text-white/50 mb-1">#{index + 1}</div>
                                <div className="flex gap-2">
                                    <button onClick={(e) => { e.stopPropagation(); duplicateFrame(index); }} disabled={frames.length >= maxFrames} className="text-xs text-white hover:text-blue-400 disabled:opacity-50 p-1 hover:bg-white/10 rounded" title="Duplicate">📋</button>
                                    <button onClick={(e) => { e.stopPropagation(); deleteFrame(index); }} disabled={frames.length <= 1} className="text-xs text-white hover:text-red-400 disabled:opacity-50 p-1 hover:bg-white/10 rounded" title="Delete">🗑</button>
                                </div>
                            </div>
                        </div>
                    ))}
                    ...
                    <button
                        ...
                        className="aspect-square border border-dashed border-white/20 flex flex-col items-center justify-center gap-1 text-white/40 hover:text-blue-400 hover:border-blue-500/50 hover:bg-blue-500/5 transition-all disabled:opacity-30 disabled:cursor-not-allowed group rounded-sm"
                    >
